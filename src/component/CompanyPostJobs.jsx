...
function CompanyPostJobs() {
  ...
  useEffect(() => {
    const fetchJobsAndCompany = async () => {
      try {
        ...
        const response = await fetch(`${BASE_URL}/job/by-company/${userId}`, fetchOptions)
        const jobList = await response.json()

        // Fetch company details (companyName, location, etc.)
        const companyResponse = await fetch(`${BASE_URL}/company/${userId}`, fetchOptions)
        ...
        const updatedJobList = jobList.map((job) => {
          ...
          const interviewers =
            Array.isArray(job.interviewPersons)
  ...
  useEffect(() => {
    if (jobs.length && jobs[selectedJob] && jobs[selectedJob].id) {
      const fetchAppliedForJob = async () => {
        try {
          const response = await fetch(`${USER_BASE_URL}/users/appliedFor/${jobs[selectedJob].id}/apply`, fetchOptions)
  ...
  const saveRecruiterEdit = () => {
    // Save changes to the main job array
    const updatedJobs = [...jobs]
  ...
  const handlePreferredQualificationChange = (index, value) => {
    const updatedPreferredQualifications = [...editedJob.preferredQualifications]
                  ...
                  <div className="flex justify-between items-center">
                    <div>
                      {editedRecruiters.length > 0 ? (
                        ...
                        <h3 className="font-medium">No recruiters assigned</h3>
                      )}
                    </div>
                    <button
                  ...
                  <div className="border border-gray-200 rounded-xl p-3 mb-4">
                    ...
                    {editedJob.preferredQualifications.map((item, index) => (
                      <div key={index} className="flex items-start mb-2">
                        <textarea
                          value={item}
                          onChange={(e) => handlePreferredQualificationChange(index, e.target.value)}
                          className="flex-1 p-2 border border-gray-300 rounded-md"
                          rows={2}
                        />
                        <button
